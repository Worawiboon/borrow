...
class CartController extends Controller
{
    ...
    public function addToCart($id){
        ...
        if ($item_exits > 0) {
            return response()->json(['message'=>'มี item นี้ในตระก้าแล้ว','status'=>302]);
        }
        ...
        return response()->json(['message'=>'เพิ่มลงตระก้าเรียบร้อยแล้ว','status'=>200]);
    }
    ...
    public function clearCart(){
        ...
        return response()->json(['message'=>'clear ตระก้าเรียบร้อยแล้ว','status'=>200]);
    }
    public function createOrder(){
        $carts = Cart::Where('user_id',auth()->user()->id)->get();
        if($carts->isEmpty()){
            return response()->json(['message'=>'ไม่พบ item ในตระก้า','status'=>422]);
        }
